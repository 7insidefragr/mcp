...
- **`check_messages` 返回一行 `[cursor-mcp-bridge:queue-empty]`**：表示插件队列里当前没有排队消息（**不是故障**）。助手**不应**把该行或「暂无消息」长篇复述给用户，也**不应**在同一回合里连环调用 `check_messages`。若你升级扩展后希望 Cursor 遵循最新规则，请在侧栏对当前工作区再点一次 **「开始配置」** 以覆盖 `.cursor/rules/` 下的规则文件。
